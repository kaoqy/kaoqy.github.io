AV.init({
  appId: '1gc0aGcTIp0nL1CM33C7aX1C-MdYXbMMI',
  appKey: '6onyEbRtxHXZGe6fZLZZ48w3',
  serverURL: 'https://1gc0agct.api.lncldglobal.com',
});
...
tjButton.addEventListener('click', () => {
  ...
  if (myname && youname && zhengwen) {
    ...
    bbqRecord.save().then(() => {
      ...
      younameInput.value = '';
      zhengwenInput.value = '';
      modalText.textContent = 'å·²åè¡¨ï¼å·æ°ç½é¡µæ¥çå¦(ï½¡â¥á´â¥ï½¡)Â ';
      modal.style.display = 'block';
      return;
    }).catch(() => {
    modalText.textContent = 'åºéå¦ï¼è¯·éè¯å¦(âäººâ)';
    modal.style.display = 'block';
    return;
    });
  } else {
    modalText.textContent = 'ä¿¡æ¯é½å¡«åäºåï¼( â¢ ÌÏâ¢Ì )â§';
    modal.style.display = 'block';
    return;
  }
});
